...
/**
 * @typedef {1|2|3} DIALOG_TYPE_VALUE
 */
 ...
 * @property {DIALOG_TYPE_VALUE} type One of `QB.chat.DIALOG_TYPE` values
...
const auth = {
  /**
   * Set current session
   * @param {QBSession} params 
   * @returns {Promise.<QBSession>}
   */
  setSession(session = {}) {
    return Auth.setSession(session)
  },
  /**
   * Get current session
   * @returns {Promise.<QBSession|undefined>}
   */
  getSession() {
...
const chat = {
   ...
   * Create dialog
   * @param {Object} params
   * @param {string} [params.name] Required if type is not CHAT
   * @param {DIALOG_TYPE_VALUE} params.type
...
const rtcconfig = {
  ...
  getAnswerTimeInterval() {
    return RTCConfig.getAnswerTimeInterval()
  },
   ...
   */
  setAnswerTimeInterval(interval) {
    if (typeof interval === 'number') {
      return RTCConfig.setAnswerTimeInterval(interval)
    } else {
  ...
  getDialingTimeInterval() {
    return RTCConfig.getDialingTimeInterval()
  },
   ...
   * @returns {Promise.<void>}
   */
  setDialingTimeInterval(interval) {
    if (typeof interval === 'number') {
      return RTCConfig.setDialingTimeInterval(interval)
    } else {
      return Promise.reject('Value should be of type "number"')
    }
  }
