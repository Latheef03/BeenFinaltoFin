...
import {
	...
	Dimensions, KeyboardAvoidingView, ToastAndroid,StyleSheet,
	Animated, StatusBar, FlatList, TouchableOpacity, Keyboard, TouchableWithoutFeedback,Platform
} from 'react-native';
...
import { deviceHeight, getTime } from '../_utils/CommonUtils';
...
export default class GetTagData extends Component {
	...
	constructor(props) {
		...
		this.arrayholder1 = [];
		// this.justCheckthedata = this.justCheckthedata.bind.this;
	}
	...
	componentWillUnmount = () =>{
	  ...
	  fetchDetails = async (responseJson) => {
		  console.log("Tagged people respones",responseJson)
	...
	likesView(data) {
		// console.log('the datas are in TP',data);
		// alert('asas')
		var data = {
	...
	_onDoubleTap = (event, data) => {
		console.log('the events double data', event, data);
		if (event.nativeEvent.state === State.ACTIVE) {
			...
			let getResult= data
			console.log("result from var",getResult)
				...
				<View style={styles.cardImage}>
					<View style={{ flexDirection: 'row', width: '100%', marginTop: index === 0 ? StatusBar.currentHeight : 0 }} >
						...
						<View style={{
							...
							{this.state.userProfileType == "1" || this.state.userProfileType == "2" ?
								<TouchableOpacity hitSlop={{ top: 10, left: 0, bottom: 10, right: 8, }} onPress={() => this.modalOpen(data)}>
					...
					>
						<View style={[styles.imageBackGroundView, { borderRadius: 15, height: hp('62%'),marginTop:10 }]}>
							{data.Image != undefined && data.Image.indexOf(".mp4") != -1 ?
								<View style={{width: "100%", height: "100%", overflow: 'hidden',flexDirection: 'column',}}>
			...
			<View style={{ flex: 1, marginTop: 0 }}>
				...
				<StatusBar translucent={true} backgroundColor="rgba(0,0,0,0)" barStyle='dark-content' />
					...
					{this.state.isLoading != true && this.state.getTagdata != null ?
						<View style={{ height:  Platform.OS == 'ios' ? deviceHeight : deviceHeight + StatusBar.currentHeight ,backgroundColor: 'black', }}>
