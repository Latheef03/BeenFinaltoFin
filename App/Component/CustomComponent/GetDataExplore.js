...
export default class GetData extends Component {
    ...
    fetchDetails = async (responseJson,data) => {
         debugger;
        console.log("Data coming from",responseJson +"and from api",data)
    ...
    for (let i = 0; i < getData.length; i++) {
      // Admin Ads Ratio
        if ((solution.length + 1) % 50 == 0) {
            solution.push(data[j]);
            solution.push(getData[i]);
            j++;
        }
      // AdMobs Ads Ratio
        else if((solution.length +1) % 5 == 0){
            solution.push(data[undefined]);
            solution.push(getData[i]);
            j++;
        }
        else {
 ...
 /* End Video Control Events */
    ...
    renderPostItem = (data, index) => {
        ...
        return (
            data === undefined ?
            <View style={{ height:500,width:'100%',marginLeft: 'auto',marginRight: 'auto',marginTop: 'auto', marginBottom:1, backgroundColor: '#fff', borderWidth:.6, borderColor:'#c1c1c1', borderRadius:25, shadowColor: "#000",shadowOffset: { width: 0, height: 3, }, shadowOpacity: 0.27,  shadowRadius: 4.65,elevation: 6,}}>
     ...
     :
       ...
       <View style={{ height: '100%', }}>
           <SelectedFilters images = {event}
                   childrenComponent = {(
         <AnimatedImage style={{ width: '100%', height: '100%', }} resizeMode={'cover'}
...
<ImageBackground borderRadius={8} style={{ width: '100%', height:360,marginTop:10}} resizeMode={'cover'}
  ...
  openAdminURl(data){
    ...
    data.Link === undefined ?
    toastMsg1("danger", "Link is not availble") :
