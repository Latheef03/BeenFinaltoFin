...
export default class LocalProfReview extends Component {
    ...
    UNSAFE_componentWillMount() {
        const getDataFromOtherProfile = this.props.route.params.data;
        this.setState({
            otherUserId: getDataFromOtherProfile.reviewData.Userid,
            ownUserPic: getDataFromOtherProfile.reviewData.LocalProfilePic,
            screenName: getDataFromOtherProfile.screenName
        })
        this.getReviews();
    }
    ...
    async getReviews() {
        ...
        var profilePicUser = await AsyncStorage.getItem("ProfilePic");
        console.log("Pic is comeing or not", profilePicUser)
        var data = {
            Userid: id,
            otherid: this.state.otherUserId
        };
        const url = serviceUrl.been_url1 + "/ReviewsAndRating";
        fetch(url, {
            method: 'POST',
            ...
            .then((responseJson) => {
                if (responseJson.status == "True") {
                    this.setState({
                        ...
                        overallReview: typeof responseJson.Avgratings !== undefined && responseJson.Avgratings.length > 0 ? responseJson.Avgratings[0].avgRate : 0,
    ...
    async postReview() {
        ...
        if (this.state.Default_Rating != "" || this.state.descrition != "") {
            ...
            var data = {
                Userid: id,
                Otherid: this.state.otherUserId,
                Ratings: this.state.Default_Rating,
                Reviews: this.state.descrition
            };
            const url = serviceUrl.been_url1 + "/PostReviewsAndRating";
            fetch(url, {
                method: 'POST',
                headers:serviceUrl.headers,
                body: JSON.stringify(data),
    ...
    async remove() {
        ...
        const url = serviceUrl.been_url1 + "/DeleteReviewsAndRating";
        fetch(url, {
            method: 'POST',
                headers:serviceUrl.headers,
                body: JSON.stringify(data),
            ...
            .then((responseJson) => {
                if (responseJson.status == "True") {
                    this.getReviews();
                    this.setState({ Default_Rating: 0 })
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1,marginTop:0,backgroundColor:'#fff' }}>
                ...
                <Toolbar {...this.props} centerTitle='Reviews' rightImgView={this.optionImg()} />
                <ScrollView scrollEnabled={true}>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent:'space-around',marginLeft:10, marginBottom: 60 }}>
                           {this.state.overallReview === 0?
                            ...
                            <View style={{ justifyContent:'center' }}>
                                <Text style={[styles.ratingText],{fontSize:20,marginLeft:10}} >{this.state.overallReview}</Text>
                                <View >
                                    <Text style={ [styles.reviewText,{fontSize:15,margin:0}]}>( {this.state.ratingsOverall} ratings )</Text>
                                </View>
                            </View>
                        </View>
                        ...
                        {this.state.getReviewList.length > 0 ?
                                    ...
                                    <View style={{ flexDirection: 'row', flex: 1, justifyContent: 'center', margin: 10 }}>
                                        ...
                                        <View style={{ width: wp(80), }}>
                                            <Text style={[Common_Style.userName, { marginLeft: 10, marginTop: 10 }]}>{item.UserName}
                                                <Text style={[Common_Style.userName, { fontFamily: Common_Color.fontLight, color: '#000' }]}>  {item.Reviews}</Text></Text>
                                        </View>
                            ...
                            <View
                                ...
                                <Text style={{ fontSize: 12, fontWeight: 'bold', color: '#000', fontFamily: Common_Color.fontLight }}>No Reviews yet!</Text>
                            </View>
...
const styles = {
    ...
    StarImage: { width:30, height: 30, borderWidth: .6,marginLeft:5  },
    childView: { justifyContent: 'center', flexDirection: 'row', marginTop: 30, },
    reviewText: { fontSize: 25, color: '#010101', margin: 4, fontFamily: Common_Color.fontRegular,fontWeight:'100' },
    ...
    ratingText: { fontSize: 25, color: '#000', fontFamily: Common_Color.fontBold, textAlign: 'center',fontWeight:'bold' }
}