...
export default class FootPrints extends Component {
    ...
    footPrintsScreen(title, datas) {
        ...
        let vlogDetails = [];
        let totalVists = [];
        footPrints.length > 0 && footPrints.map(data => {
            if (data._id == title) {
                totalDatas = data;
            }
        })

        visits.length > 0 && visits.map(data => {
            if (data._id == title) {
                totalVists = data;
            } 
        })

        // console.log('qwer-totalVists', totalVists);

        memoriesDetails = memories.length > 0 && memories.filter(m =>
            m.Location == title)

        vlogDetails = vlog.length > 0 && vlog.filter(
            ...
            <View style={{ marginTop: 0, }}>
                {this.state.footPrintsScreen === 0 ? (
                    <View style={{ width: '100%', height: '100%', backgroundColor: '#fff' }}>
                        ...
                        {/* {/ main container /} */}
                        <View>
                            <View style={{ width: wp('100%'), height: hp('100%'),marginTop:10 }}>
                               
                            {typeof footPrints !== undefined && footPrints.length > 0 ?
                                <FlatList
                                    data={footPrints}
                                    ListFooterComponent={<View style={{height:70}} />}
                                    ItemSeparatorComponent={this.seperator()}
                                    renderItem={({ item ,index}) => (
                                        <View key={`id${index}`} style={{ height: '100%', marginLeft: '3.7%', marginBottom: '3.3%', shadowColor: 'red',  }}>
                                            <TouchableOpacity activeOpacity={0.1} onPress={() => this.footPrintsScreen(item._id, item.data)}>
                                    ...
                                    <Image source={require('../../Assets/Images/Screenshot_1575975832.png')}
                                      ...
                                      resizeMode={'center'} 
                                        />
                                    <Text style={{ fontSize: 17, fontWeight: 'bold', color: '#000' }}>You have not created any Footprints Yet!</Text>
                                </View> }
                ...
                {this.state.footPrintsScreen === 1 ? (
                    ...
                    <View style={{ width: '100%', height: '100%',}}>
                        ...
                        <View style={{ flexDirection: 'row', width: '100%', height: 250 }}>
                            <ImageBackground
                                ...
                                <View style={{ flexDirection: 'row', width: '100%', bottom:0,position: 'absolute'  }} >
                                    <LinearGradient style={{ height: 35, flexDirection: 'row', width: '100%' }} colors={[ "#0f0f0f00" ,"#0f0f0f44", ]} >
                                        <View style={{ width: '100%', }}>
                                            <Text style={[Common_Style.locationText, { textAlign: 'center', marginTop: 10,fontWeight:'bold' }]}>{selectedFPTitle}</Text>
                                        </View>
                                    </LinearGradient>
                                </View>
                                ...
                                <ScrollView horizontal={true} style={{ marginLeft: wp('5%') }}>
                                    {
                                        Object.keys(selectedFPVisits).length > 0 &&
                                        //?
                                            selectedFPVisits.data.map((d,i) => {
                                                return <TouchableOpacity key={`idV${i}`} activeOpacity={0.8} onPress={()=>this.visitsViewImage(i)}>
                        ...
                        <TouchableWithoutFeedback onPress={() => this.setState({ footPrintsScreen: 0 })}>
                            <View style={{ width: 40, height: 40, top:Platform.OS === 'ios' ? StatusBar.currentHeight : StatusBar.currentHeight + 5, left: 12, position: 'absolute', }} >
                                <Image
                                    source={require(image+'Backfeed.png')}
...
const styles = StyleSheet.create(
    {
        ...
        image: { width: 30, height: 30, borderRadius: 50, borderWidth: 1, borderColor: "red", margin: '3%' }, card: { width: wp('95%'), height: hp('75'), borderWidth: 1, borderRadius: 10, borderColor: '#ddd', borderWidth: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 12 },shadowOpacity: 10, shadowRadius: 10, elevation: 4, marginLeft: 'auto', marginRight: 'auto', marginTop: 5, marginBottom: 5, backgroundColor: '#fff',},
        container2: { flexDirection: 'row', width: '95%', marginLeft: 'auto', marginRight: 'auto', marginTop: '3%' },
        icon: { width: 15, height: 20},
        footericon: { width: '23%', marginLeft: '5%' },
        fontColor: { color: '#b4b4b4' },
        fontsize: { fontSize: 10, color: '#858585' },
        footerIconImage: {width: 20, height: 20,},
        ...
        visitFont: { color: '#010101', width: wp('91%') , fontFamily: Common_Color.fontBold, fontSize: 14,fontWeight:'bold'},
        searchFollower: { flexDirection: 'row', width: wp('80%'), borderWidth: 1, borderRadius: 50, alignItems: 'center', marginTop: '3%', borderColor: '#fff', marginLeft: '10%', shadowColor: '#000', shadowOffset: { width: 0, height: 12 }, shadowOpacity: 10,shadowRadius: 10,  elevation: 4,}
    },
)
