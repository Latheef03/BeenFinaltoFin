...
import {
    ...
    View, ToastAndroid, ActivityIndicator, TouchableOpacity, FlatList, KeyboardAvoidingView,
} from 'react-native'
...
import { Common_Color, profilename, Username } from '../../Assets/Colors';
...
export default class savedpost extends Component {
    ...
    savedPost = async () => {
        ...
        const url = serviceUrl.been_url2 + '/GetSavedPosts';
        return fetch(url, {
            method: "POST",
            ...
            .then((responseJson) => {
                ...
                if (responseJson && responseJson.status == 'True') {
                    ...
                    this.setState({
                        ...
                        masterData: responseJson.CreatedSavedpost
                    })
                }
            })
            .catch((error) => {
    ...
    renderSavedPosts = () => {
        ...
        console.log('the sv post', savedPostData);
        return savedPostData.length > 0 && savedPostData.map((data, ind) => {
            ...
            let img = data.Image && data.Image.includes(',') ? data.Image.split(',') : data.Image;
            let image = img.constructor == Array ? img[0] : img;

            if (ind == 0) {
                console.log('index are 0');
                return (
                    <View key={`id${ind}`}>
                ...
                if (ind == 0) {
                    return (
                        <View key={ind.toString()} style={{
                            width: '50%', height: '100%', overflow: 'hidden', left: 0, position: 'absolute',
                        }}>
                            <SelectedFilters images={event}
                                childrenComponent={(
                                    <Image style={{ width: '100%', height: '100%' }} resizeMode="cover"
                ...
                if (ind == 1) {
                    return (
                        <View key={ind.toString()} style={{
                            width: '50%', height: '100%', overflow: 'hidden', flexDirection: 'column',
                ...
                console.log('index are less than equal 2');
                if (ind == 0) {
                    return (
                ...
                if (ind == 1) {
                    return (
                        <View key={ind.toString()} style={{
                            width: '50%', height: '50%', overflow: 'hidden', flexDirection: 'column',
                ...
                if (ind == 2) {
                    return (
                        ...
                        <View key={ind.toString()} style={{
                            width: '100%', height: '50%', overflow: 'hidden', flexDirection: 'column',
                ...
                console.log('index are less than equal 3');
                if (ind == 0) {
                    return (
    ...
    create = async () => {
        ...
        console.log('th emodal texxt', modaltext);
        if (modaltext == '') {
            this.setState({
                inputError: true
            });
        ...
        const url = serviceUrl.been_url2 + '/CreateSavedPosts';
    ...
    folderDataPass(item) {
        ...
        const { savedPostData } = this.state;
        let data = item;
        data.savedposts = savedPostData;
        // console.log('the passed data',data);
        this.props.navigation.navigate('savedpostlist', { data: data })
    }
    ...
    createFolderIconView = () => {
        ...
        if (savedPostData.length > 0) {
            return (
                ...
                //     <TouchableOpacity hitSlop={Common_Style.touchView} onPress={() => this.setState({ isModalVisible: true, inputError: false, modaltext: '' })}>
                //         <Text style={{ fontSize: 24, color: '#000', marginRight: 5, marginTop: 8 }}>+</Text>
                //     </TouchableOpacity>
                // </View>
            )
        }
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, marginTop: 0,backgroundColor:'#fff' }}>
                ...
                <Toolbar {...this.props} leftTitle='Saved Posts' rightImgView={this.createFolderIconView()} />
                {/* </View> */}
                {/* sign to all posts */}

                <View style={{ alignItems: "center" }}>
                    <TouchableOpacity onPress={() => {
                        savedPostData.length > 0 ? this.props.navigation.navigate('Allposts', { data: savedPostData })
                            : null
                    }}>
                        <View style={{
                            width: wp("84%"), height: hp("25%"), backgroundColor: "#c1c1c1",
                            borderRadius: 15, margin: 10, overflow: "hidden", borderWidth: 0.5, borderColor: "#999",
                        }} >
                ...
                <View style={{ width: wp('100%'), height: hp('100%'), flexDirection: 'column' }}>
                    ...
                    <View style={{ marginTop: "2%", width: '84%', alignSelf: 'center', }}>
                        <Text style={styles.textcolor}>All Posts</Text>
                    </View>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item, index }) => (
                                <View key={`id${index}`} style={styles.createdImagesView} >
                                    {typeof item.FeedId !== "undefined" &&
                                        item.FeedId.length > 0 ? (
                                        <TouchableOpacity onPress={() => { this.folderDataPass(item) }} >
                                            <View>
                                                {item.FeedId[0].Image.indexOf(".mp4") != -1 ? (
                                                    <View style={{
                                                        width: wp('40%'), height: hp('12%'), backgroundColor: '#c1c1c1',
                                                    ...
                                                    <View style={{
                                                        ...
                                                        <Image style={{ width: '100%', height: '100%', borderRadius: 5, backgroundColor: "#c1c1c1", }}
                                                            resizeMode="cover"
                                                            source={{ uri: serviceUrl.newsFeddStoriesUrl + item.FeedId[0].Image.split(","), }} />
                                                    </View>
                                                )}
                        ...
                        <View style={{ left: -100 }}>
                            ...
                            <View style={[Common_Style.Common_button, { width: wp(88), marginTop: 10 }]}>
                                ...
                                >
                                    <TouchableOpacity onPress={() => { creatingData ? null : this.create() }}>
                                        <Text onPress={() => { creatingData ? null : this.create() }} style={[Common_Style.Common_btn_txt, { marginTop: 12 }]}>
                                            {creatingData ? 'Creating...' : 'Create'}
                                        </Text>
                                    </TouchableOpacity>
                                </ImageBackground>
                            ...
                            <View style={[Common_Style.Common_button, { width: wp(95), marginTop: 20, marginBottom: -10 }]}>
                                <TouchableOpacity style={{ height: hp('5.7%') }} onPress={() =>
                                    ...
                                    <Text onPress={() => this.setState({ isModalVisible: false })} style={[Common_Style.Common_btn_txt, { color: 'black', marginRight: 25, color: "#fff" }]}>Cancel</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    ...
                    <View style={{ alignItems: 'center', backgroundColor: 'rgba(10,10,10,0.70)', height: '100%' }}>
                        ...
                        <View style={[Common_Style.Common_button, { width: wp(95), marginTop: 20, marginBottom: -10 }]}>
                            <TouchableOpacity style={{ height: hp('5.7%') }} onPress={() =>
                                ...
                                <Text onPress={() => this.setState({ isModalVisible: false })} style={[Common_Style.Common_btn_txt, { color: 'black', marginLeft: 5 }]}>Cancel</Text>
                            </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    textcolor: { fontFamily: Username.Font, fontSize: Username.FontSize },
    modaltext: {
        width: wp('80%'),
        height: hp('7%'),
        marginTop: '5%',
        ...
        borderColor: '#9f9f9f',
        paddingLeft: 20,
        fontFamily: profilename.Font, fontSize: profilename.FontSize
    },
    modalsImage: {
        width: wp('30%'),
        height: hp('6%'),
