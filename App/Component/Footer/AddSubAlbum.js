...
import {
    ...
    StatusBar,TextInput as RNTextInput,StatusBarIOS,
    Keyboard,
    Platform
} from 'react-native';
...
export default class AddSubAlbum extends Component {
    ...
    subAlbumView = (data,index) => {
        ...
        this.props.navigation.navigate('SubAlbumView', { sData: data })
    }
    ...
    close() {
         this.setState({albumData:null,imageView:false})
        //this.props.navigation.navigate('AddSubAlbum')
    }
       ...
       // debugger;
        ...
        this.state.images.length > 0 && this.state.images.map((data, i) => {
            ...
            data2.append("Image",
                {
                    uri: data.uri,
                    name: Platform.OS == 'ios' ? data.filename : blobName,
                    type: data.mime
                }
            )
    ...
    renderAlbSAlbums = (item,index) =>{
        ...
        return(
            <View key={`id${index}`} style={{ height: '100%', marginLeft: '3.5%', marginBottom: '3.3%', shadowOffset: { width: 5, height: 5 }, shadowColor: '#000', shadowOpacity: .2,shadowRadius:5, elevation: 8, }}>
                <TouchableOpacity activeOpacity={1} onPress={() => this.subAlbumView(item,index)}>
                    <View style={Common_Style.ShadowCurveView}>
                        {item.albumimg && item.albumimg.length > 0 ?
                            <View style={{ width: wp(45), height: hp(25), overflow: 'hidden', backgroundColor: '#c1c1c1', borderRadius: 17, shadowOffset: { width: 5, height: 5 }, shadowColor: '#000', shadowOpacity: .2,shadowRadius:5, elevation: 8, }}>
                               <ImageBackground source={{ uri: serviceUrl.album_image + item.albumimg[0].Image }}
                                   ...
                                   <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', 
                                       alignItems: 'center',shadowOffset: { width: 5, height: 5 }, shadowColor: '#000', shadowOpacity: .2,shadowRadius:5, elevation: 8, 
                            ...
                            :
                            item.albumimg && item.albumimg.length == 0 ?
                                <View style={{ width: wp(45), height: hp(25), overflow: 'hidden', backgroundColor: '#c1c1c1', borderRadius: 17, shadowOffset: { width: 10, height: 10 }, shadowColor: '#000', shadowOpacity: 1, elevation: 8, }}>
                                    <View style={{width:wp(45),height:hp(25),backgroundColor:'#00000050'}} />
                                    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', shadowOffset: { width: 10, height: 10 }, shadowColor: 'grey', shadowOpacity: 1, elevation: 8, }}>
                                        <Text style={{ color: '#FFF', fontSize: Username.FontSize, fontFamily: Username.Font }}>{item.albumName}</Text>
                                    </View>
                                </View>
                                :
    ...
    render() {
        const {isLoading,inputError,albumData,albumName,deleteModal,
          subAlbumData,subAlbum,images,imageIndex} = this.state;
        return (
                                    ...
                                    <View style={{
                                        ...
                                        <View style={{
                                            ...
                                            <RNTextInput
                                                ...
                                                // theme={{ colors: { text:'#fff', },placeholderStyle:{text:'#fff'} }}
                                                style={{
