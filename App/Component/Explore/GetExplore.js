...
import {
	...
	Dimensions, KeyboardAvoidingView, ToastAndroid,
	 StatusBar, FlatList, TouchableOpacity,TouchableWithoutFeedback,Keyboard,Platform,TextInput as RNTextInput,
	 NativeModules, StatusBarIOS,StyleSheet
} from 'react-native';
...
export default class GetExplore extends Component {
	...
	_singleTap = (data, index) => {
		console.log('the sigle tap',data);
		this.setState({
			showControl: true,
			...
			<View style={{ justifyContent: 'center', width: wp('23%'), alignSelf: 'center' }}>
				...
				<TouchableOpacity disabled={item.buttonFlag == 'sent' || item.buttonFlag == 'waiting'}
					...
					{item.buttonFlag == 'send' && (
						<View style={{
							backgroundColor: '#fb0042', paddingTop: 8, paddingBottom: 5,
							paddingLeft: 23.5, paddingRight: 23.5, borderRadius: 5,
	...
	renderPostItem = (data, index) => {
		const {showControl,paused,volumeMuted,duration,currentTime,volume,
		  singleTapPostId,iosStatusBarHeight} = this.state;
		  const  topPosition = index === 0 ? Platform.OS == 'ios' ? iosStatusBarHeight : StatusBar.currentHeight : 0
